<template>
    <section class="calendar-body">
        <div v-for="day in daysNamesList" :key="day" class="grid-cell flex-center">{{day | uppercase}}</div>
...
<script>
    ...
    import VacationCalendarDayItemComponent from './VacationCalendarDayItemComponent.vue'
        ...
        computed:{
            ...
            daysNamesList(){
                return new Array(7).fill(1).map((item, key) => {
                    return new Date(0, 0, key).toLocaleString(this.$i18n.locale(), {weekday: 'short'})
                })
            },
            datesGrid(){
                const {year, month, currentIntervalFullDate} = this.selectedDateInterval,
                    MIN_CELLS_IN_GRID = 35,
                    MAX_CELLS_IN_GRID = 42;

                function generatePrevMonth() {
                    const numberOfCurrentMonthFirstDay = new Date(year, month).getDay() - 1,
                        prevMonthDaysQuantity = getDaysInMonth(new Date(year, month - 1));

                    if (numberOfCurrentMonthFirstDay < 0) return [];

                    return eachDayOfInterval({
                        start: new Date(year, month - 1, prevMonthDaysQuantity - numberOfCurrentMonthFirstDay),
