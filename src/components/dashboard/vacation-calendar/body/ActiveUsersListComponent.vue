<template>
    <div class="active-users" :class="currentConfirmationStatus">
        <div class="user-wrapper">
            <div class="avatar">
                <i class="material-icons wait-confirm-indicator" title='Ожидается подтверждение'>access_time</i>
                <i class="material-icons check-confirm-indicator" title='День подтвержден как отпускной'>check</i>
            </div>
            <div class="user-login">{{loggedUser.login}}</div>
        </div>
        <div  class="user-wrapper" v-for="user in usersVacationsOnThisDay" :key="user.id" >
            <div class="avatar">
                <i class="material-icons" :style="{color: user.color}">account_circle</i>
            </div>
        ...
        computed:{
            ...
            usersVacationsOnThisDay() {
                return this.usersInfo.filter((user)=>{
                    return user.reserved.find((item)=> item.toString() === this.date.toString())
                })
            },
        }
    }
