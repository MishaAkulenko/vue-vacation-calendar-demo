<template>
    <div class='calendar-header'>
        <LeftSideComponent :monthsList="monthsList" :yearsList="yearsList"></LeftSideComponent>
        <RightSideComponent :monthsList="monthsList" :yearsList="yearsList"></RightSideComponent>
    </div>
</template>
        ...
        computed:{
            ...
            monthsNames(){
                return new Array(12).fill(1).map((item, month) => {
                    return new Date(0, month + 1, 0).toLocaleString(this.$i18n.locale(), {month: 'short'}).replace('.', '').toUpperCase()
                });
            },
            monthsList(){
                return this.monthsNames.map((month, key) => {
                    return {
                        value: key,
                        disabled: this.selectedDateInterval.year === todayDateYear && key < todayDateMonth,
                        name: month
                    }
                })
            },
            yearsList(){
                return new Array(YEARS_LIST_LENGTH).fill(0).map((val, key) => {
                    return {
                        value: todayDateYear + key,
                        disabled: key === 0 && this.selectedDateInterval.month < todayDateMonth,
